...
if [ $# > 1 ]; then
    ...
    echo "https://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&GLOBAL-ID=$GlobalId&SECURITY-APPNAME=$ebayAppKey&RESPONSE-DATA-FORMAT=XML$locationString&REST-PAYLOAD=true&itemFilter(0).name=ListingType&itemFilter(0).value=FixedPrice&paginationInput.entriesPerPage=$entriesPerPage&paginationInput.pageNumber=1&sortOrder=$sortOrder&keywords=$searchTerms"
    curl -s "https://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&GLOBAL-ID=$GlobalId&SECURITY-APPNAME=$ebayAppKey&RESPONSE-DATA-FORMAT=XML$locationString&REST-PAYLOAD=true&itemFilter(0).name=ListingType&itemFilter(0).value=FixedPrice&paginationInput.entriesPerPage=$entriesPerPage&paginationInput.pageNumber=1&sortOrder=$sortOrder&keywords=$searchTerms"  > temp.xml;

    # Had some problems with XML name spacing, the quickest workaround for me was to remove the xmlns definition
    sed -i.bak 's/xmlns="http:\/\/www.ebay.com\/marketplace\/search\/v1\/services"//g' temp.xml;

    # Checking that the xslt file is available
    if [ ! -f temp.xml ]; then
        echo "No results were saved in the initial xml file"
        exit
    fi

    # Checking that the xslt file is available
    if [ ! -s temp.xml ]; then
        echo "No results were saved in the initial xml file - file is empty"
        exit
    fi
    # Extract the number of pages in the result set
    totalPages=`xmllint --xpath '/findItemsAdvancedResponse/paginationOutput/totalPages/text()' temp.xml`

    # Extract the total number of entries return by the search
    totalEntries=`xmllint --xpath '/findItemsAdvancedResponse/paginationOutput/totalEntries/text()' temp.xml`
    ...
    for i in $(seq 1 $totalPages);
	    do curl -s "https://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&GLOBAL-ID=$GlobalId&SECURITY-APPNAME=$ebayAppKey&RESPONSE-DATA-FORMAT=XML$locationString&REST-PAYLOAD=true&itemFilter(0).name=ListingType&itemFilter(0).value=FixedPrice&paginationInput.entriesPerPage=$entriesPerPage&paginationInput.pageNumber=$i&outputSelector=SellerInfo&sortOrder=$sortOrder&keywords=$searchTerms" > temp$i.xml;
